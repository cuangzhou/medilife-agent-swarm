...
```bash
conda create -n medilifer1_eval python=3.11 -y
conda activate medilifer1_eval
```
...
| `--model` | `MBZUAI/MediLife-R1-8B` | Candidate model for generation |
...
> **Note:** MedEvalKit has its own dependency versions that may conflict with the `medilifer1_eval` environment. It is recommended to create a separate conda environment for it.
...
> https://medilife.cvmbzuai.com/leaderboard

Project page: https://medilife.cvmbzuai.com
...
```
MediLife_R1_8B/
├── MediLife_R1_8B.jsonl                  # Generated responses
├── MediLife_R1_8B_eval.jsonl             # Evaluation results
└── MediLife_R1_8B_score.txt              # Final score table
MediLife-R1-8B-MMMU-Medical-val/          # (included if present)
└── MMMU-Medical-val/
    ├── result.json
    ├── Basic_Medical_Science/
    │   ├── output_sample.json
