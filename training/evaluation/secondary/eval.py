...
def main():
    ...
    parser.add_argument('--model', type=str, default='MBZUAI/MediLife-R1-8B', help='Model name for generation')
    parser.add_argument('--eval_model', type=str, default=None, help='Model name for evaluation (auto-detected from config.py when judge_server=configapi)')
    parser.add_argument('--tensor_parallel_size', type=str, default=1, help='Tensor parallel size for vllm server')
    parser.add_argument('--judge_server', type=str, default='configapi', choices=['local', 'openrouter', 'configapi'], help='Judge model server: configapi (config.py API), local (vLLM), or openrouter')
    ...
    if args.evaluate:
        try:
            # Resolve eval model name: config.py for configapi, explicit arg otherwise
            if args.judge_server == 'configapi':
                from config import LLM_CONFIG
                eval_model = LLM_CONFIG["model_name"]
            else:
                eval_model = args.eval_model or "Qwen/Qwen3-14B"
            print(f"Evaluating results using model: {eval_model} (server: {args.judge_server})")
            ...
            while True:
                ...
                # Start vLLM server for local judge model (skipped when using OpenRouter or configapi)
