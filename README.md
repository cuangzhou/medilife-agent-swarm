...
- `SkillRegistry` 将本地 Skills 转换为 OpenAI function calling 工具定义，并用 Pydantic 严格校验输入与结构化结果；当前不是 MCP Server。
...
The offline capability-parity contract pins the nine packaged Skills, three worker Agents, function-calling interface, Agent Loop, Swarm, retrieval and memory adapters, DeepResearch, Trace Graph, API routes, and governed Evidence Memory invariants. Optional external services are reported as `not_testable` unless provisioned; they are never inferred from adapter availability. The parity report is a regression contract, not a new clinical-quality claim.
...
简历能力、实现入口、测试与证据边界的逐项索引见 [CAPABILITY_EVIDENCE.md](CAPABILITY_EVIDENCE.md)。
