...
class SwarmCoordinator:
    ...
    async def _process_with_swarm(
        ...
        # 返回结果
        completed_agents = list(shared_context.agent_contributions.keys())
        trace_graph = self._build_swarm_trace_graph(
            question=question,
            shared_context=shared_context,
            final_answer=final_answer,
    ...
    def _build_swarm_trace_graph(
        ...
        """Build an explainable trace graph from Swarm shared context."""
        ...
        plan_node = graph.add_node(
            "plan",
            "LeadAgent 医学任务分解",
            f"分解为 {len(shared_context.task_decomposition)} 个医学子任务",
            source="lead_agent",
            confidence=0.8,
            metadata={"timeout_occurred": timeout_occurred}
        )
        graph.add_edge(question_node, plan_node, "decomposes_to", "医学问题拆解")

        subtask_nodes: Dict[str, str] = {}
        for subtask in shared_context.task_decomposition.values():
            subtask_node = graph.add_node(
                "sub_query",
                f"{subtask.assigned_agent} 子任务",
                subtask.description,
                source="lead_agent",
                confidence=0.8,
                metadata={
            ...
            subtask_nodes[subtask.id] = subtask_node
            graph.add_edge(plan_node, subtask_node, "decomposes_to", "分配给专业 Agent")

        synthesis_node = graph.add_node(
            "synthesis",
            "LeadAgent 汇总多 Agent 结果",
            "整合专业 Agent 的医学分析、风险评估和证据检索结果。",
            source="lead_agent",
            confidence=0.7,
        ...
        for contribution in shared_context.get_contributions():
            result = contribution.result or {}
            agent_node = graph.add_node(
                "agent_result",
                f"{contribution.agent_id} 分析结果",
                str(result.get("answer", result))[:700],
                source=contribution.agent_id,
            ...
            subtask_node = subtask_nodes.get(contribution.subtask_id)
            if subtask_node:
                graph.add_edge(subtask_node, agent_node, "returns", "Agent 完成子任务")
            graph.add_edge(agent_node, synthesis_node, "supports", "支持汇总")

            if isinstance(result, dict) and result.get("trace_graph"):
                graph.merge(result.get("trace_graph"), prefix=f"{contribution.agent_id}_{contribution.subtask_id[:8]}")
        ...
        graph.add_edge(synthesis_node, final_node, "synthesizes_into", "生成最终回答")
