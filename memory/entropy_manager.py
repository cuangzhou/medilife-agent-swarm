...
class MemoryEntropyManager:
    ...
    def deduplicate_sessions(self, sessions: List[Dict[str, Any]]) -> List[Dict]:
        ...
        for session in sessions:
            ...
            content = session.get("content") or session.get("memory") or session.get("text")
            if not content:
                content = f"{question}:{summary}"
            content = " ".join(str(content).split()).strip().lower()

            # Empty legacy records must not collapse into one synthetic value.
            if not content or content == ":":
                content = f"memory_id:{session.get('memory_id') or session.get('id') or len(unique_sessions)}"
    ...
    def cleanup_old_memories(
        ...
        for memory in memories:
            ...
            # 保留最近的记忆
            cutoff_date = (
                datetime.now(timestamp.tzinfo) - timedelta(days=max_age_days)
